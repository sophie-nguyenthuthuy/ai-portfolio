"""Data loading + synthetic churn-like generator with a learnable signal.

The reference model is intentionally a small tabular churn classifier — the
*focus* of this capstone is the surrounding platform (orchestration, registry,
serving, monitoring, infra), not the model itself.
"""
...
def generate_synthetic(n: int = 4000, seed: int = 42) -> pd.DataFrame:
    """Generate a deterministic, realistic, *learnable* churn dataset.
...
def load_data(path: str | Path | None = None, n: int = 4000, seed: int = 42) -> pd.DataFrame:
    """Load reference data from CSV if present, else fall back to synthetic."""
    if path is not None:
        p = Path(path)
        if p.exists():
